...
async function downloadImage(url: string, index: number): Promise<{ buffer: Buffer, extension: string, name: string } | null> {
  try {
  ...
  for (const url of matches) {
    ...
    try {
      ...
      const file = await storage.createFile(
        BUCKET_ID,
        ID.unique(),
        InputFile.fromBuffer(Buffer.from(arrayBuffer), `${imgData.name}.${imgData.extension}`)
      );
